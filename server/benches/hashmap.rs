...
fn one_reader_one_stream(c: &mut Criterion, concurrent: usize) {
    ...
    let group_name = format!("HashMap-Parking-Lot-{}", concurrent);
    let mut group = c.benchmark_group(group_name);
    let shards_configs = [1, 2, 64, 128, 512, 1024, 4096, 8192];
    let segments = 5;
    let tracks = 5;
...
fn make_map(
    ...
    Arc<Vec<String>>,
    Arc<HashMap<u64, Mutex<HashMap<String, u64>>>>,
) {
    ...
    for i in 0..shards {
        let shard: HashMap<String, u64> = HashMap::new();
    ...
    for stream in &stream_names {
        for t in 0..tracks {
            for s in 0..segments {
                ...
                let shard = shard(key.as_str(), shards);
                map.get(&shard).unwrap().lock().insert(key, shard);
            }
        }
    }
